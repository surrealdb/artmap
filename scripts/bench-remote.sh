...
REMOTE_DIR="Repositories/artmap"
...
if [[ "$MODE" == "--artmap" ]]; then
    echo "--- Running ArtMap multi-threaded benchmarks ---"
    ssh "${REMOTE_HOST}" "bash -lc 'source ~/.cargo/env && cd ${REMOTE_DIR} && cargo bench --bench artmap_bench'"
fi
